...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    NSURL *launchDeeplink = nil;
    NSDictionary *activityDict = launchOptions[UIApplicationLaunchOptionsUserActivityDictionaryKey];
    NSUserActivity *activity = activityDict[@"UIApplicationLaunchOptionsUserActivityKey"];
    if ([activity.activityType isEqualToString:NSUserActivityTypeBrowsingWeb] && activity.webpageURL) {
        launchDeeplink = activity.webpageURL;
    } else if ([launchOptions[UIApplicationLaunchOptionsURLKey] isKindOfClass:[NSURL class]]) {
        launchDeeplink = launchOptions[UIApplicationLaunchOptionsURLKey];
    }
    if (launchDeeplink && [appConfig applyLaunchDeeplink:launchDeeplink]) {
        self.handledLaunchDeeplink = launchDeeplink;
    }
...
{
    if (self.handledLaunchDeeplink &&
        [userActivity.webpageURL isEqual:self.handledLaunchDeeplink]) {
        self.handledLaunchDeeplink = nil; // one-shot
        return YES;
    }
